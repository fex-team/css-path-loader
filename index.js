  ...
  for (var i in css) {
    ...
    if (css[i] === '}') {
      ...
      if (rawFragment.indexOf('@') >= 0) {
        atFragment.push(rawFragment);
      }
      start = i + 1;
    }
  }

  atFragment.forEach(function (at, a_index) {
    cssFragment.forEach(function (css, css_index) {
        cssFragment[css_index] = css.replace(at, '');
    });
  });
